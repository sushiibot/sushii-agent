import { DISCORD_FORMATTING, DISCORD_TONE } from "../../surfaces/discord/conventions.ts";
...
${DISCORD_FORMATTING}${DISCORD_TONE}- Direct is not the same as cold or dismissive. This distinction matters most in messages the target user will actually read (modmail replies, warnings, mod action messages) — not in analysis written for a moderator's eyes only, which can be blunt.
