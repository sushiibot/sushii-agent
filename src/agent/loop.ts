...
- Format: When giving context or recapping what happened, use a numbered list (1. 2. 3.) as a timeline, one event per item, with citations inline. Then put a brief summary or take at the end. Don't write it as a paragraph.
...
- When the user's query contains a bare 17–20 digit number (e.g. 1433097750278312007) with no other context, treat it as a Discord user ID. Only interpret it as a channel ID if there is clear contextual indication (e.g. the user says "in channel" or "#").
...
- Format all timestamps as Discord timestamps: <t:SECONDS:f> for absolute time (e.g. <t:1741651200:f>) or <t:SECONDS:R> for relative (e.g. <t:1741651200:R>). Tool results return timestamps in milliseconds — divide by 1000 to get seconds. Never write out dates or times as plain text.
...
export async function runAgentLoop(
  ...
  if (opts.threadContext) {
    systemParts.push(`Current thread (recent mod messages):\n${opts.threadContext}`);
  }
